...
void MakeDepthFrame(string color_path, string depth_path, float param_depth_scale, float *dev_depthIntegration, uchar *colorIntegration)
{
    ...
    //读入到CPU
    cv::Mat color_bgr = cv::imread(color_path, CV_LOAD_IMAGE_UNCHANGED);
    if (color_bgr.empty())
    {
    ...
    // }
    cv::Mat depth_uchar = cv::imread(depth_path, CV_LOAD_IMAGE_UNCHANGED);
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < poses.size(); i += iterStep)
    {
        ...
        if (true) {
            for (unsigned int i = 0; i < numIter; i++) {    //对输入的深度图erode两回，并存回d_depthInputRaw中
                if (i % 2 == 0) {
